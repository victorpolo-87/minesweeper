...
public class VentanaJuego extends JFrame {
    ...
    private void actualizarBotones() {
        ...
        boolean juegoPerdido = juego.getEstadoJuego() == EstadoEnum.PERDIDO;
        int bombaX = juego.getBombaExplotadaX();
        int bombaY = juego.getBombaExplotadaY();
        ...
        for (int fila = 0; fila < x; fila++) {
            for (int columna = 0; columna < y; columna++) {
                ...
                if (juegoPerdido && casilla instanceof CasillaBomba) {
                    if (fila == bombaX && columna == bombaY) {
                        boton.setIcon(imgBombExploded); // Bomba que explotó
                    } else if (casilla.marcada) {
                        boton.setIcon(imgBombFlaggedLost); // Bombas marcadas
                    } else {
                        boton.setIcon(imgBomb); // Bombas no marcadass
                    }
                } else if (casilla.marcada) {
